...
### 2.2 推荐验证拓扑（第一版，已修正）
*   **VBUS 路径**：验证板需要一个 `SYS_5V` 轨道给 ESP32 供电。`SYS_5V` 必须同时支持两种来源：
    *   无外部电源时：`Tab_A9_VBUS -> 隔离二极管/理想二极管 -> SYS_5V -> ESP32_5V`，由平板 Host 供电给 ESP32。
    *   有外部电源时：`VBUS_IN -> 隔离二极管/理想二极管 -> SYS_5V -> ESP32_5V`，由充电器供电给 ESP32。
*   **平板充电注入路径**：`VBUS_IN -> 肖特基/限流/eFuse -> Tab_A9_VBUS`，尝试让平板在 Host 模式下接受外部 VBUS。
...
3.  **Tab_A9_VBUS** 通过第二路隔离二极管/理想二极管连接到 **SYS_5V**，保证无外部电源时平板能给 ESP32 供电。
4.  **VBUS_IN** 通过第三路隔离二极管/理想二极管连接到 **SYS_5V**，保证有外部电源时充电器给 ESP32 供电。
5.  **SYS_5V** 连接到 **ESP32_5V_IN**。
6.  **Tab_A9_CC1** 和 **Tab_A9_CC2** 各接一个 **5.1kΩ** 电阻到 **GND**（预留断开跳线）。
7.  **Tab_A9_D+** 直连 **ESP32_D+**。
