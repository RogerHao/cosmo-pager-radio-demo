...
### 2.2 推荐验证拓扑（第一版，已修正）
*   **最小拓扑**：`Tab_A9_VBUS` 直接作为 `SYS_5V` 给 ESP32 供电。`VBUS_IN -> SS34 -> Tab_A9_VBUS/SYS_5V`。无外部电源时，平板 Host 给 ESP32 供电；有外部电源时，外部 5V 通过 SS34 尝试注入同一节点。该拓扑只需要 1 颗 SS34。
*   **完整诊断拓扑**：验证板单独定义一个 `SYS_5V` 轨道给 ESP32 供电。`SYS_5V` 同时支持两种来源：
    *   无外部电源时：`Tab_A9_VBUS -> SS34/理想二极管 -> SYS_5V -> ESP32_5V`，由平板 Host 供电给 ESP32。
    *   有外部电源时：`VBUS_IN -> SS34/理想二极管 -> SYS_5V -> ESP32_5V`，由充电器供电给 ESP32。
*   **平板充电注入路径**：`VBUS_IN -> SS34/限流/eFuse -> Tab_A9_VBUS`，尝试让平板在 Host 模式下接受外部 VBUS。
...
3.  **无外部电源 OTG 测试**：断开外部 5V，确认平板 `Tab_A9_VBUS` 能通过隔离路径拉起 `SYS_5V`，ESP32 能上电并枚举为 HID。
4.  **VBUS 冲突测试（关键）**：断开外部 5V，万用表测量外部充电输入端，确认平板输出的 VBUS 没有反灌到充电器输入。接入外部 5V 后，确认 `SYS_5V` 来源切换不导致 ESP32 掉电。
5.  **枚举中断测试**：在 ESP32 正常工作时，插拔外部 5V 电源，观察 ESP32 的 HID 连接是否会断开重连。
...
3.  **最小拓扑**：**Tab_A9_VBUS** 直接连接到 **ESP32_5V_IN**，此时 **Tab_A9_VBUS** 同时也是 **SYS_5V**。
4.  **完整诊断拓扑**：**Tab_A9_VBUS** 通过第二路 SS34/理想二极管连接到 **SYS_5V**，保证无外部电源时平板能给 ESP32 供电。
5.  **完整诊断拓扑**：**VBUS_IN** 通过第三路 SS34/理想二极管连接到 **SYS_5V**，保证有外部电源时充电器给 ESP32 供电。
6.  **SYS_5V** 连接到 **ESP32_5V_IN**。
7.  **Tab_A9_CC1** 和 **Tab_A9_CC2** 各接一个 **5.1kΩ** 电阻到 **GND**（预留断开跳线）。
8.  **Tab_A9_D+** 直连 **ESP32_D+**。
9.  **Tab_A9_D-** 直连 **ESP32_D-**。
10.  所有设备的 **GND** 共地。
