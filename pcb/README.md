...
| 连接器布局 | 左上 J4 NFC 8P / 右上 J3 Action Button 2P / 左中 J1 EC11 左 5P / 右中 J2 EC11 右 5P |
...
## 下一阶段：PCB 落版（Plan v2，2026-05-18 锁定）

详细方案见 `~/.claude/plans/iridescent-snuggling-planet.md`。要点：

- 把万能板飞线版 1:1 复制到 KiCad（连接器位置、网络名都已经在飞线上验证过）
- ESP32-S3 DevKitC N16R8 用**两条 1×22 单排母**（不是 2×22 双排，单排好焊好对齐），间距 22.86mm
- **5 路 XH2.54 连接器**（J1 EC11-L / J2 EC11-R / J3 BTN / J4 NFC / J5 OTG USB 数据）
...
| J1 | EC11-L (用户左旋钮) | XH2.54 5P | 左旋钮 | A=42, B=41, SW=40, GND, GND |
| J2 | EC11-R (用户右旋钮) | XH2.54 5P | 右旋钮 | A=17, B=18, SW=8, GND, GND |
| J3 | BTN | XH2.54 2P | Kailh BOX 按钮 | SIG=1, GND |
| J4 | NFC | XH2.54 8P | RC522 mini | CS=16, SCK=15, MOSI=7, MISO=6, IRQ=5, GND, RST=4, 3V3 |
| J5 | OTG USB 数据 | XH2.54 4P | OTG dongle (YK16-09E V1) 拆 USB-A 后 4 飞线 | VBUS, D-=19, D+=20, GND |
...
| Ref | 元件 | 封装 | 说明 |
|-----|------|------|------|
| U1A, U1B | 1×22P 排母座 (×2) | 2.54mm THT | DevKitC 插入用 |
| J1, J2 | XH2.54 5P 立式直针座 (×2) | THT | EC11 旋钮 |
| J3 | XH2.54 2P 立式直针座 | THT | Action Button |
| J4 | XH2.54 8P 立式直针座 | THT | RC522 NFC |
...
| H1-H4 | M2.5 安装孔 | 通孔 | 四角 |

**取消的元件**（vs 旧 spec）：USB-C 母座 J5/J6、USB-C CC 5.1k Rd × 4、SS34 防倒灌、WS2812B LED 连接器 J4 (旧)、330Ω LED 限流。所有 USB 充电由 OTG dongle 内部承担，LED 用 DevKitC GPIO48 板载 RGB。
...
- 连接器位置就近其需要的 GPIO（最短走线）：
  - **PCB 左侧**：J5 USB (靠后, GPIO 19/20) / J1 EC11-L (中段, GPIO 42/41/40) / J3 BTN (靠前, GPIO 1)
  - **PCB 右侧**：J4 NFC (中段, GPIO 4-7/15-16) / J2 EC11-R (中段偏上, GPIO 17/18/8)
- R/C 去抖网络紧贴 J1/J2 的 A/B pin（电容回 GND 路径最短）
