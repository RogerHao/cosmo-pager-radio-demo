...
- ESP32-S3 DevKitC 用 2×22P 排母座座（不焊死，方便拆换）
- 4 路 XH2.54 连接器（J1/J2/J3/J4）
- OTG 一分二子板的安装区 + 短线焊盘 + 应力释放
- EC11 改为外加 10kΩ 上拉（飞线用的内部 ~45kΩ 噪声较多）
- NFC SPI 时钟可尝试拉回 5MHz（铜箔走线 + 去耦电容）
- `sdkconfig.defaults` 升 16MB flash + octal PSRAM
...
3. 需要购买并拆解 2-3 个 OTG + 充电转接器，选择可内置的小 PCB。
...
2. 买 2-3 个 OTG + 充电转接器，复测稳定性并筛选最适合拆解的一款。
3. 拆解最稳定、体积最小的转接器，记录内部 PCB 尺寸、芯片丝印、焊点定义。
